    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i + 1]]
        "*** YOUR CODE HERE ***"
        positions = [max(b, default=None, key=lambda i: b[i]) for b in livingGhostPositionDistributions]
        ...
        for act in legal:
            min_distance = min(
                [self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, act), g_pos) for g_pos in
                 positions])

            if min_distance < _value:
                _value = min_distance
                _action = act
