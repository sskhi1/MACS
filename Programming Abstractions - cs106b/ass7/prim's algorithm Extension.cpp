...
Set<Edge> createMaze(int numRows, int numCols) {
	...
	Loc start = mazeGrid[0][0];
	Loc next1 = mazeGrid[0][1];
	Loc next2 = mazeGrid[1][0];
	Edge e1 = makeEdge(start, next1);
	Edge e2 = makeEdge(start, next2);

	pq.enqueue(e1, randomReal(0, 1));
	pq.enqueue(e2, randomReal(0, 1));
	visited.add(start);
	Set<Edge> pqueue;
	pqueue.add(e1);
	pqueue.add(e2);
		...
		if(!visited.contains(end)){
			...
			for(int i = row - 1; i < row + 2; i++){
				for(int j = col - 1; j < col + 2; j++){
					if(mazeGrid.inBounds(i, j)){
						if((i == row - 1 && j == col - 1) || (i == row + 1 && j == col + 1) || (i == row - 1 && j == col + 1)
							|| (i == row + 1 && j == col - 1) || (i == row && j == col)) continue;
							Loc nxt = mazeGrid[i][j];
							Edge e = makeEdge(end, nxt);
							if(pqueue.contains(e)) continue;
							pqueue.add(e);
