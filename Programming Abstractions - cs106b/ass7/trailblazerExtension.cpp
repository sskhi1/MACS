...
void constrMazePQ(int numRows, int numCols, Grid<Loc> & mazeGrid, PriorityQueue<Edge> & pq){
...
Set<Edge> createMaze(int numRows, int numCols) {
	...
	constrMazePQ(numRows, numCols, mazeGrid, pq);
	
	UnionFind unionFind(numRows*numCols);

	while(!unionFind.isSingleComponent()){
		Edge e = pq.dequeue();
		if(!unionFind.connected(e.start.row * numRows + e.start.col,
								e.end.row * numRows + e.end.col)){
			// If the endpoints of e are not in the same cluster: 
			// Merge the clusters containing the endpoints of e.
			unionFind.merge(e.start.row * numRows + e.start.col,
								e.end.row * numRows + e.end.col);
			maze.add(e);
		}
	}
