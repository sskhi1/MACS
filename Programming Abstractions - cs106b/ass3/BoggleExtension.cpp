...
void addRandomU(Grid<char> &board, int row, int col);
void findQletters(Grid<char> &board);
void displayBoard(Grid<char> &board);
void myTurn(Grid<char> &board, Lexicon &lex, Set<string> &myWords, Set<string> &userWords);
void computerTurn(Grid<char> &board, Lexicon &lex, Set<string> &myWords, Set<string> &userWords);
bool wordIsOnTheBoard(string word, Grid<char> &board, int row, int col, bool haveFirst, Set<int> &path);
void findRemainingWords(Player &comp, Grid<char> &board, Lexicon &lex, Set<string> &myWords, string soFar, int row, int col, Set<int> computerPath, string &secretStr, int &k, Set<string> &userWords);
void finalChance(string secretStr, Grid<char> &board);
/* Main program */
...
void initializeBoggle(){
	...
	while(true){
		...
		myTurn(board, lex, myWords, userWords);
		cout << endl;
		cout << "Computer will wind all the remaining words now." << endl;
		cout << endl;
		computerTurn(board, lex, myWords, userWords);
		cout << endl;
	}
	cout << "The End" << endl;
}
...
Grid<char> drawInitBoggleBoard(int size){
	...
	if(configuration == "y"){
		...
		while(true){
			// computer generates board configuration again if player wants to.
			letters = computerGenerateLetters(size);
			boardAddLetters(board, letters);
			displayBoard(board);
			findQletters(board);
			string prompt = getLine("Do you like this configuration? press 'y' to stop generating: ");
			if(prompt == "y") break;
		}
	}
	return board;
...
// Q is largely useless unless it is adjacent to a U.
void findQletters(Grid<char> &board){
	for(int i = 0; i < board.numRows(); i++){
		for(int j = 0; j < board.numCols(); j++){
			if(board[i][j] == 'Q'){
				addRandomU(board, i, j);
			}
		}
	}
}
...
void addRandomU(Grid<char> &board, int row, int col){
	for(int i = row - 1; i < row + 2; i++){
		for(int j = col - 1; j < col + 2; j++){
			if((i == row && j == col) || !board.inBounds(i, j)) continue;
			// 'u' is next tu 'q' with 70% probability.
			if(randomChance(0.7)){
				labelCube(i, j, 'U');
			}
		}
...
void computerTurn(Grid<char> &board, Lexicon &lex, Set<string> &myWords, Set<string> &userWords){
	...
	for (int i = 0; i < board.numRows(); i++){
        for (int j = 0; j < board.numCols(); j++){
			...
			findRemainingWords(comp, board, lex, myWords, "", i, j, computerPath, secretStr, k, userWords);
...
void findRemainingWords(Player &comp, Grid<char> &board, Lexicon &lex, Set<string> &myWords, string soFar, int row, int col, Set<int> computerPath, string &secretStr, int &k, Set<string> &userWords){
	...
	if(soFar.length() > 3){
		if(lex.contains(soFar) && !myWords.contains(soFar)){
			// If computer finds tooooooo many words (more than 75) it is nearly impossible to beat it...
			// So, we give the player a chance. If you guess our secret word, you don't lose.
			if(!userWords.contains(soFar) && k == 75){
				secretStr = soFar;
				k++;
			}
			k++;
	...
	for(int i = row - 1; i < row + 2; i++){
		for(int j = col - 1; j < col + 2; j++){
			if(!board.inBounds(i, j) || (i == row && j == col) || computerPath.contains(i * 10 + j)){
				...
				computerPath.add(i * 10 + j);
				findRemainingWords(comp, board, lex, myWords, soFar + board[i][j], i, j, computerPath, secretStr, k, userWords);
				computerPath.remove(i * 10 + j);
			}
		}
	}
...
void finalChance(string secretStr, Grid<char> &board){
	cout << endl;
	cout << "Computer guessed too many words... Fortunately it gave you chance:" << endl;
	cout << "The computer didn't guess one secret word! If you guess this one, you win!" << endl;
	cout<< endl;
	Set<int> path;
	string guess = getLine("Your guess: ");
	if(guess == secretStr){
		if(wordIsOnTheBoard(guess, board, 0, 0, false, path))
		cout << "Congratulations! You guessed the secret word! To me you are the W-est person on earth <3" << endl;
	}else{
		cout << "Unfortunately, you didn't guess the secret word... Better luck next time :(" << endl;
	}
}
