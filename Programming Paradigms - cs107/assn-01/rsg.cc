...
// Generate random sentence using recursion.
void generateSentenceRec(map<string, Definition>& grammar, vector<string>& sentence, string beginningOfSentence){
	Definition def = grammar[beginningOfSentence];
	Production prod = def.getRandomProduction();
	
	vector<string>::iterator it = prod.begin();
	vector<string>::iterator end = prod.end();
	for(; it != end; it++){
		string nextWord = *it; // iterators behave like pointers
		if(nextWord[0] == '<') generateSentenceRec(grammar, sentence, nextWord); // nonterminal 
		else sentence.push_back(nextWord);
	}
}
...
void generateSentence(map<string, Definition>& grammar){
	string beginningOfSentence = "<start>"; 
	vector<string> sentence;
	generateSentenceRec(grammar, sentence, beginningOfSentence);
...
int main(int argc, char *argv[])
{
  ...
  // first change on github attempt...
